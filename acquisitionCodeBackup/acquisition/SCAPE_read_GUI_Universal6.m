function varargout = SCAPE_read_GUI_Universal6(varargin)
...
% Last Modified by GUIDE v2.5 31-Jul-2017 10:03:21
...
if strcmp(scanName,'.')==0 && strcmp(scanName,'..')==0
    ...
    try
        ...
        handles.info.info.cal.ylat = 1.23 ;
        handles.info.info.cal.zdep = 1.03;
        handles.info.info.cal.xfact = 300;
        handles.info.info.cal.xwid = handles.info.info.cal.xfact*handles.info.info.daq.scanAngle/handles.info.info.daq.pixelsPerLine;
        handles.info.info.cal2 = sprintf('Get proper cal vals! Using xwid=%0.2f ylat=1.23 zdep=1.03', handles.info.info.cal.xwid);
        infotext{8} = sprintf('Calibration: %s',handles.info.info.cal2);
        infotext{9} = sprintf('Field of view: y: %.2f  x: %.1f z: %.1f um',handles.info.info.cal.ylat*handles.info.zyla.numLatPix, handles.info.info.cal.xwid*handles.info.info.daq.pixelsPerLine,handles.info.zyla.numDepths*handles.info.info.cal.zdep);
...
%         parfor_progress(0);
    ...
    % if handles.nooutput == 0; %temp edit KP
...
%                 parfor_progress(0);
            ...
            if get(handles.OpeninFiji,'Value')
                ...
                addpath('/usr/local/MATLAB/R2016a/java/')
                Miji;
                if numel(ss) == 3
                    numtiffs = 1;
                else
...
addpath('C:\Users\kripp\Documents\Fiji.app\scripts\')
addpath('C:\Program Files\MATLAB\R2016b\java')
Miji;
