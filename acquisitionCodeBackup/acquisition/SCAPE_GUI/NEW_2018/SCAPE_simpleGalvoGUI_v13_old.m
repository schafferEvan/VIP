...
% Last Modified by GUIDE v2.5 07-Feb-2018 17:51:15
...
if (1 == get(handles.use_stim_checkbox, 'Value'))
handles.aoStimSession.Rate = handles.info.camera.framerate;
end
...
if(inPreview == 1)
    ...
    %set filter wheel to position selected for alignment mode
    handles.originalfiltval = get(handles.filter_wheel, 'Value');
    if get(handles.highpower, 'Value')
        set(handles.filter_wheel, 'Value', 8);
        input = 'pos=8';
...
galvo_offset_orig = str2num(get(handles.galvo_um_offset, 'String'));
...
galvo_offset = galvo_offset_orig + (info.daq.scanFOV-scanFOV)/2-xypos(2);


imagesc(y,z,log(double(squeeze(max(SCAPE_data(:,5:end-5,:),[],3))')));
axis image
colormap gray
xlabel('Y (pixels)')
ylabel('Z (pixels)')
...
width = ceil(xypos(3));
height = xypos(4);
heighttop = height/2-xypos(2);
% adjust height to center around cente rof camera chip with max required vertical offset
realheight = max([(info.camera.yROI/2-heighttop),(height-(info.camera.yROI/2-heighttop))])*2;
...
if(isHR == 1)
    ...
    set(handles.y_ROI, 'String', num2str(realheight));
    ...
    set(handles.y_ROI2, 'String', num2str(realheight));
