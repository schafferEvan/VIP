...
% Last Modified by GUIDE v2.5 05-Dec-2017 03:19:35
...
if (str2num(get(handles.ScaleG,'String'))~=1)
    ...
    %         green = imresize3(green,[scaleval*size(green,1) size(green,2) size(green,3)]);
    %     else
    %         green = imresize3(green,[scaleval*size(green,1) scaleval*size(green,2) size(green,3)]);
    %     end
...
if get(handles.MIPview,'Value')== 1
    clear rgb
    rgb(:,:,1) = squeeze(max(scr*(double(red1(:,round(end/2):round(2*end/3),:,1))-minr),[],2))';
    rgb(:,:,2) = squeeze(max(scg*(double(green1(:,round(end/2):round(2*end/3),:,1))-ming),[],2))';
    rgb(:,:,3) = zeros(size(rgb(:,:,1)));
    axis(handles.MainFigureSC)
    handles.RGBMIPS = rgb;
    setappdata(handles.MainFigureSC,'fig',handles.RGBMIPS);
...
if handles.ss(4)==1
    ...
    for i = 1:ss(3)
        if separateTiffs
            ...
            imwrite(rgb, imgToSave, 'tif', 'Compression', 'none', 'WriteMode', 'Append');
        end
    end
