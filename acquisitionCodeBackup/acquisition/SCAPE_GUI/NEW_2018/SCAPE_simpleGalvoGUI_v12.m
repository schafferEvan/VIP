function varargout = SCAPE_simpleGalvoGUI_v12(varargin)
% SCAPE_SIMPLEGALVOGUI_V12 MATLAB code for SCAPE_simpleGalvoGUI_v12.fig
% %      SCAPE_SIMPLEGALVOGUI_V12, by itself, creates a new SCAPE_SIMPLEGALVOGUI_V12 or raises the existing
% %      singleton*.
%
%      H = SCAPE2_GALVO_GUI_V15 returns the handle to a new SCAPE_SIMPLEGALVOGUI_V12 or the handle to
%      the existing singleton*.
%
%      SCAPE_SIMPLEGALVOGUI_V12('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SCAPE_SIMPLEGALVOGUI_V12.M with the given input arguments.
%
%      SCAPE_SIMPLEGALVOGUI_V12('Property','Value',...) creates a new SCAPE_SIMPLEGALVOGUI_V12 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
% %      applied to the GUI before SCAPE_simpleGalvoGUI_v12_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
% %      stop.  All inputs are passed to SCAPE_simpleGalvoGUI_v12_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SCAPE_simpleGalvoGUI_v12

% Last Modified by GUIDE v2.5 08-Jan-2018 11:59:25

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @SCAPE_simpleGalvoGUI_v12_OpeningFcn, ...
    'gui_OutputFcn',  @SCAPE_simpleGalvoGUI_v12_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before SCAPE_simpleGalvoGUI_v12 is made visible.
function SCAPE_simpleGalvoGUI_v12_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for SCAPE_simpleGalvoGUI_v12
...
% UIWAIT makes SCAPE_simpleGalvoGUI_v12 wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = SCAPE_simpleGalvoGUI_v12_OutputFcn(hObject, eventdata, handles)
...
handles.framerateChanged = addlistener(handles.frameRateFile,'Changed', ...
    @(hObject,eventdata)SCAPE_simpleGalvoGUI_v12('frame_rate_changed',hObject,eventdata,handles));
...
if(inPreview == 1)
    ...
    set(handles.filter_wheel, 'Value', 10);
    input = 'pos=10';
    fprintf(handles.filterWheelObj, input);
