function varargout = SCAPE_splitcolsV9b(varargin)
% SCAPE_SPLITCOLSV9B MATLAB code for SCAPE_splitcolsV9b.fig
%      SCAPE_SPLITCOLSV9B, by itself, creates a new SCAPE_SPLITCOLSV9B or
%      raises the existingparfor
%      singleton*.
%
%      H = SCAPE_SPLITCOLSV9B returns the handle to a new SCAPE_SPLITCOLSV9B or the handle to
%      the existing singleton*.
%
%      SCAPE_SPLITCOLSV9B('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SCAPE_SPLITCOLSV9B.M with the given input arguments.
%
%      SCAPE_SPLITCOLSV9B('Property','Value',...) creates a new SCAPE_SPLITCOLSV9B or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SCAPE_splitcolsV9b_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SCAPE_splitcolsV9b_OpeningFcn via varargin.
%
...
% Edit the above scalered3 to modify the response to help SCAPE_splitcolsV9b

% Last Modified by GUIDE v2.5 30-Mar-2018 13:16:40

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @SCAPE_splitcolsV9b_OpeningFcn, ...
    'gui_OutputFcn',  @SCAPE_splitcolsV9b_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before SCAPE_splitcolsV9b is made visible.
function SCAPE_splitcolsV9b_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SCAPE_splitcolsV9b (see VARARGIN)

% Choose default command line output for SCAPE_splitcolsV9b

answer = questdlg('Load data from Workspace?','answers','Yes','No, load raw','No, load raw');

switch answer
    case 'Yes'
        handles.inputdata = evalin('base', 'SCAPE_data');
        handles.readgui.info.info = evalin('base', 'info');
        handles.readgui.info.zyla = evalin('base', 'zyla');
        handles.readgui.directory = handles.readgui.info.info.dataDirectory;
        handles.readgui.secs = 'all';
    ...
    case 'No, load raw'
        ...
        %sequence
        if framesPerScan == 2 || framesPerScan == 1
...
function varargout = SCAPE_splitcolsV9b_OutputFcn(~, ~, handles)
...
if handles.ss(4)==1
    ...
    if get(handles.MIP,'Value')
        ...
        tmpg = max(max(green(:,:,:),[],3)-min(green(:,:,:),[],3),[],2);vi
...
transforms.ss = size(handles.inputdata);
transforms.scanFOV = handles.readgui.info.info.daq.scanFOV;
...
handles.proc.rgb.xr2 = transforms.xr;
handles.proc.rgb.yr2 = transforms.yr;
handles.proc.rgb.yg2 = transforms.yr;
handles.proc.rgb.zr2 = transforms.zr;
handles.proc.rgb.zg2 = transforms.zg;
