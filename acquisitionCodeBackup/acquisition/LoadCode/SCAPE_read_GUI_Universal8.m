function varargout = SCAPE_read_GUI_Universal8(varargin)
% SCAPE_read_GUI_Universal8 MATLAB code for SCAPE_read_GUI_Universal8.fig
%      SCAPE_read_GUI_Universal8, by itself, creates a new SCAPE_read_GUI_Universal8 or raises the existing_tiff
%      singleton*.
%
%      H = SCAPE_read_GUI_Universal8 returns the handle to a new SCAPE_read_GUI_Universal8 or the handle to
%  java -versio    the existing_tiff singleton*.
%
%      SCAPE_read_GUI_Universal8('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SCAPE_read_GUI_Universal8.M with the given input arguments.
%
%      SCAPE_read_GUI_Universal8('Property','Value',...) creates a new SCAPE_read_GUI_Universal8 or raises the
%      existing_tiff singleton*.  Starting from the left, property value gridpairs are
%      applied to the GUI before SCAPE_read_GUI_Universal8_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SCAPE_read_GUI_Universal8_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SCAPE_read_GUI_Universal8

% Last Modified by GUIDE v2.5 01-Mar-2018 17:35:34

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @SCAPE_read_GUI_Universal8_OpeningFcn, ...
    'gui_OutputFcn',  @SCAPE_read_GUI_Universal8_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before SCAPE_read_GUI_Universal8 is made visible.
function SCAPE_read_GUI_Universal8_OpeningFcn(hObject, ~, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SCAPE_read_GUI_Universal8 (see VARARGIN)
% set(handles.foldername, 'String', 'F:/');
% handles.initialfolder = '/local_mount/space/enterprise/4/SCAPE_data_backup/';
currentFolder = pwd;
addpath(currentFolder);

handles.initialfolder = 'E:\';
...
guidata(hObject,handles);
% Choose default command line output for SCAPE_read_GUI_Universal8
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes SCAPE_read_GUI_Universal8 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SCAPE_read_GUI_Universal8_OutputFcn(~, ~, handles)
    ...
    if isfield(handles,'tiff')
        if handles.tiff == 1
            ...
            if get(handles.OpeninFiji,'Value')
                addpath('C:\Users\Axel-SCAPE\Documents\fiji-win64\Fiji.app\scripts\')
...
% handles    structure with handles and user data (see GUIDATA)
loaddata_Callback(hObject, eventdata, handles)
SCAPE_quickview_VV2(evalin('base', 'SCAPE_data'),evalin('base', 'info'))
