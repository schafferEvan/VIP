function varargout = SCAPE_quickview_VV_KP4_eh(varargin)
% SCAPE_QUICKVIEW_VV_KP4_EH MATLAB code for SCAPE_quickview_VV_KP4_eh.fig
%      SCAPE_QUICKVIEW_VV_KP4_EH, by itself, creates a new SCAPE_QUICKVIEW_VV_KP4_EH or raises the existing
%      singleton*.
%
%      H = SCAPE_QUICKVIEW_VV_KP4_EH returns the handle to a new SCAPE_QUICKVIEW_VV_KP4_EH or the handle to
%      the existing singleton*.
%
%      SCAPE_QUICKVIEW_VV_KP4_EH('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SCAPE_QUICKVIEW_VV_KP4_EH.M with the given input arguments.
%
%      SCAPE_QUICKVIEW_VV_KP4_EH('Property','Value',...) creates a new SCAPE_QUICKVIEW_VV_KP4_EH or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SCAPE_quickview_VV_KP4_eh_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SCAPE_quickview_VV_KP4_eh_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SCAPE_quickview_VV_KP4_eh

% Last Modified by GUIDE v2.5 28-Jul-2018 13:38:04

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @SCAPE_quickview_VV_KP4_eh_OpeningFcn, ...
    'gui_OutputFcn',  @SCAPE_quickview_VV_KP4_eh_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before SCAPE_quickview_VV_KP4_eh is made visible.
function SCAPE_quickview_VV_KP4_eh_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SCAPE_quickview_VV_KP4_eh (see VARARGIN)

% Choose default command line output for SCAPE_quickview_VV_KP4_eh
...
% UIWAIT makes SCAPE_quickview_VV_KP4_eh wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = SCAPE_quickview_VV_KP4_eh_OutputFcn(hObject, eventdata, handles)
...
if get(handles.xyzview,'Value')==1
    ...
    if ~handles.RGB
        if get(handles.df,'Value')
            dfrange = str2num(get(handles.dfrange,'String'));
            if(~logscale)
                imagesc(handles.y, handles.x, max(double(squeeze(handles.data(:,handles.zslice,:,handles.t))-double(squeeze(max(mean(handles.data(:,handles.zslice,:,dfrange),4))))), [], 2)');
            else
                imagesc(handles.y, handles.x, log10(max(double(squeeze(handles.data(:,handles.zslice,:,handles.t)))-double(squeeze(max(mean(handles.data(:,handles.zslice,:,dfrange),4)))), [], 2)'));
                %       imagesc(handles.y, handles.x, log10(double(squeeze(max(handles.data(:,handles.zslice,:,handles.t))))-double(squeeze(mean(handles.data(:,handles.zslice,:,dfrange),4))), [], 2))');
...
switch mipval
    case 1
        dataToWrite = permute(squeeze(max(handles.data(2:end-2,2:end-2,5:end-5,:), [], 2)), [2 1 3]);
        fileName = [handles.scanName '_XYMIP.tif'];
    case 2
        dataToWrite = permute(squeeze(max(handles.data(2:end-2,2:end-2,5:end-5,:), [], 3)), [2 1 3]);
