...
% Last Modified by GUIDE v2.5 31-Aug-2017 16:01:31
...
if strcmp(scanName,'.')==0 && strcmp(scanName,'..')==0
    ...
    if exist([calpath 'SCAPE_calibrations.txt'],'file')==2
        ...
        if success ==1
            ...
            handles.info.info.cal.xwid = handles.info.info.cal.xfact*handles.info.info.daq.scanAngle/handles.info.info.daq.pixelsPerLine;
            handles.info.info.cal2 = sprintf('Get recent vals! Using xwid=%0.2f ylat=1.39 zdep=1.14', handles.info.info.cal.xwid);
...
if isempty(volumesToLoad)==0
    ...
    if get(handles.checkbox2,'Value')
        ...
        green_y_project=max(SCAPE_data(1:(end/2),:,:,floor(sn(4)/2)),[],3);
        red_y_project=max(SCAPE_data((end/2)+1:end,:,:,floor(sn(4)/2)),[],3);
