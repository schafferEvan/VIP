...
global m traces im H
x = round(mean(H.indicator.XData));
y = round(mean(H.indicator.YData));
patch = im(y-5:y+5,x-5:x+5,:);
traces.isImagingOn = squeeze(max(max(patch,[],2),[],1));
m.traces = traces;
...
[w,~,nframe]=size(im);
tStep = tStep-1;
H.im.CData = im(:,:,tStep); %imshow(im(:,:,tStep));
...
function playF
global im tStep isActive playFobj H
[w,~,n]=size(im);
...
function playR
global im tStep isActive playRobj H
[w,~,n]=size(im);
...
function ffF
global im tStep isActive ffFobj H
[w,~,n]=size(im);
while isActive && tStep<n
    st = min(10,n-tStep);
    tStep = tStep+st;
...
function ffR
global im tStep isActive ffRobj H
[w,~,n]=size(im);
while isActive && tStep>1
    st = min(10,tStep-1);
    tStep = tStep-st;
    H.im.CData = im(:,:,tStep); %psz(im(:,:,tStep));
    text(20,w-20,['Frame ',num2str(tStep),' of ',num2str(n)],'color',[1 1 1],'fontweight','bold');
    updateObjectsAndTraces(-st);
    drawnow
end
...
global editROI waitForPress lsf tStep indicator
global videoFolder files isavi im m traces H
...
% make indicator
t = 0:2*pi/20:2*pi;
X = 50 + 8*sin(t); Y = 50 + 8*cos(t); Z = X*0;
H.indicator = patch(X,Y,Z,'r');
H.indicator.FaceAlpha = .3;
moveit2(H.indicator);
