...
function extractBehavior_feedTouch(date,behaviorFolder)
% THIS IS A RETROFIT TO LOG THE TIME OF FIRST CONTACT WITH SUGAR
...
isavi = true;
if isavi
    files = dir([videoFolder,'f*run2.avi']);
else
    files = dir([videoFolder,'f*run2.mp4']);
end
...
stimObj = uicontrol('Style', 'togglebutton', 'String', 'stim detected',...
    'Units','normalized',...
    'Fontsize',14,...
    'ForegroundColor',[0 0 0],...
    'Position', [.875 .2 .1 .08],...
...
viewROI = [0,0,0,0]; %roiTemplate.viewROI;
legs = [0,0,0,0]; %roiTemplate.legs; %zeros(1,4);
try
...
for n = 1:length(files)
    ...
    m = matfile( [movfile(1:end-4),'_touch.mat'],'Writable',true);
    aviobj = VideoReader(movfile);
...
save( [videoFolder,files(vidNum).name(1:end-4),'_touch.mat'],'viewROI','legs');
% vidNum = vidNum+1;
